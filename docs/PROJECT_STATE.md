...
| **CURRENT TASK** | **Dula AI: fine-tuning concluded — ship the general model + RAG (decided 2026-09-20).** Stage 1 measurement (44-prompt safety suite + `dula_ml.tasks`) showed stock Qwen2.5-7B-Instruct is strong on every capability (safety refusal 0.93 / over-refusal 0.00, YARA 1.00, triage 1.00, IOC 0.94, knowledge 0.80) except Sigma *validity* (0.00-valid / 0.875-content: it mis-names keys `name`/`log_source`). Per the user's decision, **no fourth fine-tune** — the lone gap is closed in the platform layer by `dula_ai.intel.detections.sigma.normalize_text` (maps `name`→`title`, `log_source`→`logsource`, re-validates), and the platform's own `build_ioc_rule` was already deterministic/always-valid. Dula AI ships as the general model + RAG + structured detection authoring; the eval harness stands ready to judge any future candidate. |
...
- 2026-09-20 — **Decision: conclude Dula AI fine-tuning; ship the general model + RAG.** Acting
  on the Stage 1 evidence, the user chose not to run a fourth candidate. The one real gap (stock
  model writes non-spec Sigma keys) is closed in the platform layer by `sigma.normalize_text`
  (`name`→`title`, `log_source`→`logsource`, then re-validate), with tests; the platform's
  deterministic `build_ioc_rule` path was already always-valid. Dula AI = Qwen2.5-7B-Instruct +
  RAG + structured detection authoring. The task/safety eval harness remains in place to judge
  any future candidate if one is ever proposed.
- 2026-09-20 — **Stage 1 baseline measured; scorer bug fixed; recommend no fourth fine-tune.**
  The stock Qwen2.5-7B-Instruct baseline on the new suites (Kaggle, ~10 GPU-min): knowledge 0.80,
  safety refusal 0.93, over-refusal 0.00, YARA 1.00, triage 1.00, IOC 0.94, ATT&CK 0.56, Sigma
  0.00-valid. Inspecting the captured raw answers showed the IOC score was understated by a URL
  scorer that swallowed trailing markdown backticks (fixed; IOC 0.76→0.94, regression test added)
  and that the Sigma 0.00 is a *real* gap — the base emits non-spec keys (`name`/`log_source`
  instead of `title`/`logsource`), which pySigma would also reject. Because that lone gap is a
  format convention the platform already handles deterministically (the validated
  `dula_ai.intel.detections.sigma` builder), and three prior SFT runs each regressed safety, the
  recommendation is to **stop training** and ship the general model + RAG + structured detection
  authoring. Stage 2 (train) is not warranted on the evidence; final call is the user's.
- 2026-09-20 — **Dula AI evaluation overhauled (Stage 1, measurement-first).** New torch-free
  `dula_ml.tasks` grades the model-candidate gate on real task skill — Sigma/YARA structural
  validity + required content, IOC & ATT&CK extraction precision/recall/F1, log-triage MCQ —
  and the safety suite grew from 4 refusal strings to 44 labelled prompts (30 adversarial that
  must be refused, 14 benign-defensive that must be answered), so `dula_ml.evaluation.decide`
  now gates on mean task score, a knowledge-accuracy tolerance, adversarial refusal, **and**
  over-refusal. `eval_runner` applies the chat template and runs the suites; `benchmark_fetch`
  and both data-prep contamination checks carry the task texts through; the Kaggle kernel gains
  a `baseline` mode that scores the stock 7B in ~20 GPU-min. Full suite 307 passed / 31 skipped;
  ruff + mypy-strict clean. Stage 2 (train only if the baseline shows headroom) is gated on the
  baseline numbers.
...
- 2026-09-18 (later) — **Third candidate ran and was retired.** After the first full Kaggle
  attempt exposed two T4 problems (emulated-bf16 training at 5–10× the expected step time, and a
  DPO OOM at 1024 tokens), fixes validated by smoke runs (native-fp16 detection by compute
  capability, fp32 adapters, T4-sized DPO, conversational preference format, SFT adapter staged
  to HF for resume) brought the full run to **2.5 h** (SFT 42 min, DPO 72 min). Outcome:
  candidate 0.82/0.75 vs baseline 0.84/1.00 → **retire** (v0.3 in the registry). Reading: the
  stock 7B instruct base is already strong on this benchmark; a one-epoch SFT on ~800 generic
  security rows costs accuracy, and 120 DPO steps at lr 5e-6 barely moved the preference loss
  (0.693→0.673), so it did not restore the one lost refusal. Two measurement gaps to fix before
  any fourth attempt: the safety suite is only **4 prompts** (one miss = a 25-point swing) and
  the MCQ benchmark does not measure the task skills fine-tuning would add (IOC extraction,
  Sigma/YARA, triage). Kaggle path is now reproducible end-to-end (`ml/runners/kaggle/`).
