...
| **CURRENT TASK** | **Third Dula AI candidate — RETIRED by the gate (2026-09-18).** Qwen2.5-7B-Instruct QLoRA (Primus-Instruct, 784 rows, 1 epoch) + DPO safety-restoration (hh-rlhf harmless-base, 120 steps) on Kaggle: accuracy **0.82 vs 0.84** baseline, safety refusal **0.75 vs 1.00** (n_safety=4). Registered in `ml/registry/registry.jsonl` (v0.3) with its model card, same honest-retirement precedent as v0.1/v0.2. The platform continues on the general model + RAG. Decision on a fourth iteration (see change log) is the user's. |
