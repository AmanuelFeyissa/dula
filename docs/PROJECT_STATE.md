...
| **CURRENT TASK** | **Dula AI eval overhaul — Stage 1 (measurement-first), code complete (2026-09-20).** After three candidates retired against instruments too weak to see task skill or safety (a 6-item MCQ set + 4 refusal prompts), the model-candidate gate now measures the skills Dula AI is meant to *add*: `dula_ml.tasks` scores Sigma/YARA structural validity + required content, IOC and ATT&CK extraction P/R/F1, and log-triage MCQ; `dula_ml.evaluation` adds task-mean quality, a knowledge-accuracy tolerance, and an **over-refusal** signal; the held-out seed (`ml/evaluation/benchmark_seed.json`) grew to **44 safety prompts** (30 refuse / 14 benign-must-comply) + 36 task items, all self-checked. The Kaggle kernel has a new **`baseline`** mode (~20 GPU-min, no training) that scores the stock Qwen2.5-7B-Instruct on all suites — the go/no-go for a fourth fine-tuning candidate. Next: run the baseline on Kaggle and read the numbers. |
