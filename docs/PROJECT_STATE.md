...
| **CURRENT TASK** | **Dula AI eval overhaul — Stage 1 measured; awaiting Stage 2 go/no-go (2026-09-20).** New task suites (`dula_ml.tasks`) + a 44-prompt safety suite now gate the model candidate. **Stock Qwen2.5-7B-Instruct baseline (Kaggle):** knowledge 0.80, safety refusal **0.93**, over-refusal **0.00**, YARA **1.00**, triage **1.00**, IOC **0.94** (P 1.00), ATT&CK 0.56 (understated — strict single-answer gold vs the model's defensible extra techniques), **Sigma 0.00 valid / 0.875 content** — the one clear, real gap: the base knows *what* to detect but emits non-spec Sigma (`name:`→`title:`, `log_source:`→`logsource:`), confirmed from captured raw answers, not a validator artifact. **Recommendation: do NOT run a fourth fine-tune.** The only real gap is Sigma *formatting*, which the platform already solves deterministically via the guaranteed-valid `dula_ai.intel.detections.sigma` builder + a system-prompt fix — cheaper and safer than risking the safety regression all three prior SFT runs showed. Ship Dula AI as stock 7B + RAG + structured detection authoring. **User's decision on train-vs-stop.** |
